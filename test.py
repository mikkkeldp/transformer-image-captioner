...
def greedy_search(image, caption, cap_mask, id):
    ...
    for i in range(config.max_position_embeddings - 1): #iterate rows of vocab_size x max_pos_embedding matrix
        predictions = model(image, caption, cap_mask, id, True, config.ml, config.ml_type)
...
"""
def beam_search(model, image, caption, cap_mask, k, ids):

    sequences = []
    predictions = model(image, caption, cap_mask, ids, True, config.ml, config.ml_type)
    ...
    for i in range(1, config.max_position_embeddings -1):
        ...
        for j in range(k): #for every beam in sequence list
            ...
            if new_complete:
                ...
                new_predictions = model(image, new_caption, new_cap_mask, ids, True, config.ml, config.ml_type)
...
@torch.no_grad()
def evaluate(model,image, caption, cap_mask, id):
    model.eval()
    if config.beam_width == 1:
        return greedy_search(model, image, caption, cap_mask, id)
    else:
        return beam_search(model, image, caption, cap_mask, config.beam_width, id)
...
def test(config):
    ...
    for img in tqdm(testing_imgs):
        ...
        output = evaluate(model,sample, caption, cap_mask, id)
